...
| 工具数 | 15（共享） | 23（+5 本地 +3 委派） | 15（三层权限） |
...
| 21 | `delegate_to_research` | 委派数据收集任务给 Research Sub-Agent | 同步 | CLI |
| 22 | `delegate_to_analysis` | 委派深度分析任务给 Analysis Sub-Agent | 同步 | CLI |
| 23 | `delegate_to_trading` | 委派交易决策任务给 Trading Sub-Agent | 同步 | CLI |
...
3. 工具结果做智能摘要而非粗暴截断：
   - 诊断工具 → 保留 `code`、`phase`、`health`、`trigger_signals` 等关键字段
   - 行情工具 → 保留最近 5 条数据
